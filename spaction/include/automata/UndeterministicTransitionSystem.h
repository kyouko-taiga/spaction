...
class UndeterministicTransitionSystem;

template<typename Q, typename S>
class UndeterministicTSIterator : public ITransitionBaseIterator<Q,S,UndeterministicTSIterator<Q,S>> {
public:
    explicit UndeterministicTSIterator(UndeterministicTransitionSystem<Q, S> *transition_system,
                                       const Q *q, const S *s) :
    ...
    explicit UndeterministicTSIterator(typename std::unordered_map<S, std::vector<const Transition<Q, S>*>>::iterator end) :
    _it(end), _end(end),
    _transition_it(typename std::vector<const Transition<Q, S>*>::iterator()) { }

    virtual bool
    is_equal(const ITransitionBaseIterator<Q,S,UndeterministicTSIterator<Q,S>> & rhs) const {
        const UndeterministicTSIterator& bi = static_cast<const UndeterministicTSIterator&>(rhs);
        bool   b = _it == bi._it;
    ...
    virtual ITransitionBaseIterator<Q,S,UndeterministicTSIterator<Q,S>> *clone() const {
        return new UndeterministicTSIterator(*this);
    }
...
template<typename Q, typename S>
class UndeterministicTransitionSystem :
    public TransitionSystem<Q, S, UndeterministicTransitionSystem<Q,S>, UndeterministicTSIterator<Q, S>> {
public:
    using super_type = TransitionSystem<Q, S, UndeterministicTransitionSystem<Q,S>, UndeterministicTSIterator<Q, S>>;
    using TransitionBaseIterator = UndeterministicTSIterator<Q, S>;
    friend class UndeterministicTSIterator<Q, S>;
private:
    class StateBaseIterator : public IStateBaseIterator<Q, S> {
     public:
        ...
        virtual bool is_equal(const IStateBaseIterator<Q,S> & rhs) const {
        ...
        virtual const IStateBaseIterator<Q,S> & operator++() {
            ++_it;
            return *this;
        }

        virtual IStateBaseIterator<Q,S> *clone() const {
 ...
 public:
    explicit UndeterministicTransitionSystem(std::shared_ptr<Data> d):
        super_type(new DumbControlBlock<Transition<Q, S>>(), d) {}
 ...
 protected:
    ...
    virtual ITransitionBaseIterator<Q,S,TransitionBaseIterator> *_successor_begin(const Q &state,
                                                                                      const S *label) {
        return new TransitionBaseIterator(this, &state, label);
    }

    virtual ITransitionBaseIterator<Q,S,TransitionBaseIterator> *_successor_end(const Q &state) {
        return new TransitionBaseIterator(_graph[state].end());
    }

    /// @note This method is not implemented yet.
    virtual ITransitionBaseIterator<Q,S,TransitionBaseIterator> *_predecessor_begin(const Q &state,
                                                                                        const S *label) {
        return nullptr;
    }

    /// @note This method is not implemented yet.
    virtual ITransitionBaseIterator<Q,S,TransitionBaseIterator> *_predecessor_end(const Q &state) {
        return nullptr;
    }

    virtual IStateBaseIterator<Q,S> *_state_begin() {
        return new StateBaseIterator(this);
    }
    virtual IStateBaseIterator<Q,S> *_state_end() {
