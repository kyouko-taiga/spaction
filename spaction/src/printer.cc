  ...
  void visit (const binop * node) final
  {
    ...
    switch (node->get_type ())
    {
      ...
      case COST_RELEASE:
        ...
        std::cerr << "we should not encounter a cost release during instantiation" << std::endl;
        exit (1);
        break;
    }
...
cltl_formula *
instantiate (const cltl_formula * f, int n)
{
  // n > 0
  instant v (n);
  f->accept (v);
  cltl_formula * res = v.get_result ();
  return res;
}
